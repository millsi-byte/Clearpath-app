...
export default function App() {
  ...
  async function checkCode() {
    ...
    try {
      const res = await sbFetch(`access_codes?code=eq.${encodeURIComponent(codeInput.trim().toUpperCase())}&select=code,used`);
      const data = await res.json();
      if (!data?.length) { setCodeError("Code not found. Check your confirmation email."); setCodeLoading(false); return; }
      if (data[0].used) { setCodeError("This code has already been used."); setCodeLoading(false); return; }
